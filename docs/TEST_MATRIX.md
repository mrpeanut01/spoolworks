...
| Vendor catalogue | 194 records across eight brands, ids unique | unit | pass |
| Vendor catalogue | every id published in 0.5.0 still names the same filament | regression | pass |
| Vendor catalogue | collapsed printer variants record the disagreement they resolved; one-profile brands have none | unit | pass |
| Vendor catalogue | every record is physically plausible (density, temp range, diameter) | unit | pass |
